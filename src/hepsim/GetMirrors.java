...
This function also look at the mirrors. 
...
public class GetMirrors {
	...
	/**
	* Start downloading and asking questions about mirrors. 
	       * @param inputURL input HepSim URL
	       * @param dest destination
	**/
	public GetMirrors(final String inputURL, final File dest) {
	...
	public static void main(String args[]){
		...
		if (surl_array != null){
			if (surl_array.length>1){
				...
				// kb.close();
				surl=surl_array[num1];
				System.out.println("Download URL: "+surl);
				System.out.print("Press any key to continue. Press 'n' to cancel and exit:");
				boolean yn = false;
				String answer = ((kb.nextLine()).trim()).toLowerCase();
			      	if (answer.equals("n"))  {
                                        System.out.println("Exit..");
                                        System.exit(0);
                                } else System.out.println("Start downloading.. ");
				kb.close();


			} // end multiple mirrors
		} // not null
